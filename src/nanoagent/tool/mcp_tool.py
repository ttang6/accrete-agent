"""MCPTool —— 把一个 MCP server 的工具 auto-expand 成 nanoagent 的独立 BaseTool。
...
异步桥同步（关键摩擦点）：fastmcp.Client 是纯异步，而 nanoagent 的 BaseTool._execute
...
def register_mcp_server(registry, server_name: str, server_config: dict) -> int:
    ...
    registry：nanoagent 的 ToolRegistry。
