...
- 不进 user-turn 热路径——经 debug 通道手动触发（`python -m
  accrete.evolution.runtime_memory.methodology_distiller`），攒够才落。
...
from accrete.evolution.runtime_memory.lesson_score import is_active
...
def main() -> int:
    ...
    from accrete.runtime.subagent import build_subagent_llm

    min_lessons = int(os.getenv("ACCRETE_METHODOLOGY_MIN_LESSONS", str(DEFAULT_MIN_LESSONS)))
