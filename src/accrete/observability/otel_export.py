...
守卫:`opentelemetry` 未装 或 `ACCRETE_OTEL` 未设 → 导出静默 no-op,不影响主流程。
激活:`uv pip install -e ".[otel]"` + `ACCRETE_OTEL=1`(+ 可选 exporter 配置,见 configure)。
"""
...
def is_enabled() -> bool:
    """导出是否该跑：`ACCRETE_OTEL` 已设 且 opentelemetry 可导入。"""
    return bool(os.getenv("ACCRETE_OTEL")) and _otel_available()
...
def emit_run(
    ...
    *,
    ...
    try:
        ...
        tracer = provider.get_tracer("accrete.otel_export")
...
def _maybe_mark_error(span, output) -> None:
    ...
    if not isinstance(output, str):
        return
    try:
        from accrete.runtime.tool_failure import is_tool_failure
...
def configure(exporter: str = "console", endpoint: Optional[str] = None):
    ...
    provider = TracerProvider(resource=Resource.create({"service.name": "accrete"}))
    if exporter == "otlp":
