...
class TextFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        name = record.name.replace("accrete.", "")
...
def _ensure_handlers() -> None:
    ...
    root = logging.getLogger("accrete")
...
def get_logger(name: str) -> logging.Logger:
    ...
    Args:
        name: 模块名，如 "react_agent"、"memory"、"rag"。
              实际创建的 logger 名为 "accrete.{name}"。
    """
    _ensure_handlers()
    return logging.getLogger(f"accrete.{name}")
...
def log_event(event: str, level: int = logging.INFO, **fields: Any) -> None:
    ...
    from accrete.core.metrics import observability_enabled
...
class RunTracer:
    ...
    def step_error(self, exc: BaseException, **extra) -> None:
        ...
        **下游消费者为后续扩展预留**（ReflexionGate + consolidator），
        ...
        """
        from accrete.core.error_classifier import classify_error
    ...
    def _maybe_emit_otel(self) -> None:
        ...
        守 ACCRETE_OTEL + opentelemetry 可导入；否则静默 no-op。fail-open：任何异常
        不影响 trace 落盘 / 主流程。内部 jsonl（喂飞轮/eval）不受影响。
        """
        try:
            from accrete.observability import otel_export
